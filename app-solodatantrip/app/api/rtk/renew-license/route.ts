...
import { renewRtkLicenseAction } from "@/lib/rtk/actions";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await renewRtkLicenseAction(body.plan, clientIp);
    ...
    return NextResponse.json(
      { success: false, error: "Falha ao renovar licença RTK." },
      { status: 500 },
    );
  }
}
